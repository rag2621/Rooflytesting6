...
const dest = new CloudinaryStorage({
  cloudinary,
  params: {
    folder: "cloud_uploads", // 👈 naya Cloudinary folder, replace with your choice
    allowed_formats: ["jpg", "jpeg", "png"],
    public_id: (req, file) => Date.now() + "-" + file.originalname,
  },
});

const up = multer({ storage: dest });
...
app.post("/verify-kyc", up.single("document"), async (req, res) => {
  const { name, dob, idNumber, Email, Type } = req.body;
  ...
  try {
    ...
    if (extractedDob) {
      const dateParts = extractedDob.split(/[-/]/);
...
console.log(idNumber);
    ...
    const isIDValid = extractedText.includes(idNumber) || extractedText.includes(formattedNumber);

    console.log("Formatted Date:", formattedDate);
    console.log("Formatted Aadhaar:", formattedNumber);
    console.log("isDOBValid:", isDOBValid);
    console.log("isIDValid:", isIDValid);

    if (isDOBValid && isIDValid) {
      await kyc.create({ ID: idNumber });

      if (Type === "broker") {
        await brok.updateOne({ Email }, { $set: { KYC: "Approved" } });
      } else {
        await Schema.updateOne({ Email }, { $set: { KYC: "Approved" } });
      }
...
app.get("/check/delete/:email",async(req,res)=>{
  const {email}=req.params;
  const uu= await Schema.findOne({Email:email});
  if(!uu){
...
const PORT = 5500;
const MONGO_URI = "mongodb+srv://rooflyestates:12345@roofly.qkt6j7c.mongodb.net/"
